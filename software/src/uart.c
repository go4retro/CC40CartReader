...
#if defined UART0_RX_BUFFER_SHIFT && UART0_RX_BUFFER_SHIFT > 0
  /* Return number of characters waiting in the buffer. */
  return ( rx0_head == rx0_tail ) ? 0 :
         ( rx0_head  > rx0_tail ) ? (rx0_head - rx0_tail) :
         ( (rx0_head + sizeof(rx0_buf)) - rx0_tail );
#else
  /* Without buffers, only a single character can be read at a time. */
  return ((UCSRAA & (1 << RXCA)) != 0 ? 1 : 0);
#endif
}
